...
- Use `.node-version`, disable setup-node's pre-bootstrap package-manager cache probe, install the exact npm version from `packageManager`, and use a normal `npm ci`.
